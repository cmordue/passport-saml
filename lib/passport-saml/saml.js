...
SAML.prototype.initialize = function (options) {
  ...
  if (!options.nameIDField) {
    options.nameIDField = 'nameID';
  }

  if (!options.sessionIndexField) {
    sessionIndexField = 'sessionIndex';
  }
...
SAML.prototype.generateLogoutRequest = function (req) {
  ...
  var request = {
    'samlp:LogoutRequest' : {
      ...
      'saml:NameID' : {
        '@Format': req.user.nameIDFormat || this.options.identifierFormat,
        '#text': req.user[this.options.nameIDField]
      }
    }
  };

  if (this.options.sessionIndexField && req.session[this.options.sessionIndexField]) {
    request['samlp:LogoutRequest']['samlp:SessionIndex'] = {
      '#text': req.session[this.options.sessionIndexField]
    };
  }
